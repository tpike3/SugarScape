...
class NetAgent(Agent):
    ...
    def update_trades(self, partner):
        '''
        Helper function for agents to keep track of trades
        
        '''
        
        if partner in self.model.trades[self].keys(): 
            self.model.trades[self][partner] += 1
            self.model.trades[partner][self] += 1
        else: 
            self.model.trades[self][partner] = 1
            self.model.trades[partner][self] = 1
